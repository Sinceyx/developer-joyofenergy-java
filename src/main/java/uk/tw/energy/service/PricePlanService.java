...
public class PricePlanService {
    ...
    public Optional<Map<String, BigDecimal>> getConsumptionCostOfElectricityReadingsForEachPricePlan(String smartMeterId) {
        ...
        return Optional.of(pricePlans.stream().collect(Collectors.toMap(PricePlanPo::getPlanName, t -> calculateCost(electricityReadings.get(), t.toPricePlan()))));
    }
    ...
    private BigDecimal calculateAverageReading(List<ElectricityReading> electricityReadings) {
        BigDecimal summedReadings = electricityReadings.stream().map(ElectricityReading::getReading).reduce(BigDecimal.ZERO, (reading, accumulator) -> reading.add(accumulator));
